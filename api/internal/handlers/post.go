...
func (h *PostHandler) Update(c *gin.Context) {
    ...
    if err := h.db.Transaction(func(tx *gorm.DB) error {
        if err := tx.Model(&post).Updates(updates).Error; err != nil { return err }
        if body.Tags != nil {
            oldTagIDs := make([]string, 0, len(post.Tags))
            for _, t := range post.Tags {
                oldTagIDs = append(oldTagIDs, t.ID)
            }
            if err := tx.Where("post_id = ?", post.ID).Delete(&models.PostTag{}).Error; err != nil { return err }
            if err := attachHumanTags(tx, &post, body.Tags); err != nil { return err }
            if err := recalcTagCounters(tx, oldTagIDs...); err != nil { return err }
        }
        return nil
    }); err != nil {
        serverError(c, err)
...
func (h *PostHandler) Delete(c *gin.Context) {
    ...
    _ = h.db.Transaction(func(tx *gorm.DB) error {
        ...
        if err := tx.Where("post_id = ?", post.ID).Delete(&models.PostTag{}).Error; err != nil { return err }
        if err := tx.Delete(&post).Error; err != nil { return err }
...
func (h *PostHandler) Vote(c *gin.Context) {
    ...
    var body struct { Value int `json:"value" binding:"required,min=-1,max=1"` }
    if err := c.ShouldBindJSON(&body); err != nil {
...
func attachHumanTags(tx *gorm.DB, post *models.Post, raw []string) error {
    ...
    for _, name := range names {
        ...
        if err := tx.First(&tag, "slug = ?", slug).Error; err != nil {
            ...
            tag.LastUsedAt = now
            tag.UpdatedAt = now
            if err := tx.Model(&tag).Updates(map[string]interface{}{"name": tag.Name, "last_used_at": now, "updated_at": now}).Error; err != nil { return err }
        }
        if err := tx.Create(&models.PostTag{PostID: post.ID, TagID: tag.ID, CreatedAt: now}).Error; err != nil { return err }
        if err := recalcTagCounters(tx, tag.ID); err != nil { return err }
    }
    return nil
}
